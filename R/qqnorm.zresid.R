...
qqnorm.zresid <- function (Zresidual, irep=1, diagnosis.test = "SW",
                           ...
                           outlier.return=TRUE, outlier.value = 3, outlier.set = list(), legend.settings = list(), ...)
{
  ...
  for (i in irep) {
    ...
    legend.args1 <- modifyList(default.legend1, args)
    legend.args2 <- modifyList(default.legend2, args)
