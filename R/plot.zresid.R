...
plot.zresid <- function(Zresidual, irep = 1:ncol(Zresidual), ylab = "Z-Residual",
  ...
  if (!is.null(category)) {
    ...
    if (type %in% c("count")) {
      ...
      unique.cats <- unique(c("count","zero")[seq_along(Zresidual) %in% attr(Zresidual, "zero_id") + 1])
  ...
  for (j in irep) {
    ...
    default.legend <- list(legend = unique.cats, col = unique(col),
                           pch = unique(pch), cex = 0.8, xpd = TRUE, bty = "n",
    ...
    if (X == "index") {
      ...
      if (!is.null(test.legend)) do.call(legend, c(list(x = plot_limits[2] - (plot_limits[2] - plot_limits[1]) * 0.05, y = plot_limits[4] * 0.7), test.legend))
    ...
    if (X == "fitted.value") {
      ...
      do.call(legend, c(list(x = plot_limits[2] - (plot_limits[2] - plot_limits[1]) * 0.01, y = plot_limits[4]), legend.args))
      if (!is.null(test.legend)) do.call(legend, c(list(x = plot_limits[2] - (plot_limits[2] - plot_limits[1]) * 0.05, y = plot_limits[4] * 0.7), test.legend))
      if (isTRUE(outlier.return)) {
        if (!identical(id.outlier, integer(0))) {
          # Recalculate circles based on current par("usr")
    ...
    if (X != "index" & X != "fitted.value") {
      ...
      if (!is.null(fitted.value)) {
        ...
        if(!is.null(args[["log"]])) plot_limits[names(plot_lim_convert)==args[["log"]]] <- 10^(plot_limits[names(plot_lim_convert)==args[["log"]]])
        do.call(legend, c(list(x = plot_limits[2], y = plot_limits[4]), legend.args))
        if (!is.null(test.legend)) do.call(legend, c(list(x = plot_limits[2] - (par("usr")[2] - par("usr")[1]) * 0.03, y = plot_limits[4] * 0.7), test.legend))
        if (isTRUE(outlier.return)) {
          if (!identical(id.outlier, integer(0))) {
            # Recalculate circles based on current par("usr")
            outlier.circles <- rep((par("usr")[2] - par("usr")[1]) * 0.03, length(id.outlier))
